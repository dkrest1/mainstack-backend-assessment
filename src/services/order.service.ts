...
class OrdersService {
  ...
  async getOrders(page: number, limit: number, status: string | undefined): Promise<PaginateResult<ORDERS>> {
    const options: PaginateOptions = {
      page,
      limit,
      populate: [{ path: 'customer_id', populate: 'avatar' }],
    }
  ...
  ): Promise<PaginateResult<ORDERS_DETAILS>> {
    const options: PaginateOptions = {
      page,
      limit,
      populate: [{ path: 'product_id', populate: 'image_id' }, { path: 'order_id' }],
    }
  ...
  async findOrder(orderId: string): Promise<ORDERS> {
    ...
    const populateOrder = await OrdersModel.populate(order, [{ path: 'customer_id', populate: 'avatar' }])
  ...
  async updateOrder(orderId: string, updateOrderRequest: UpdateOrderRequest): Promise<ORDERS_DETAILS | null> {
    ...
    const updatedOrderDetails = await OrderDetailsModel.findOneAndUpdate({ order_id: orderId }, updateOrderDetails, {
      new: true,
    })
